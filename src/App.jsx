...
export default function BasketballGame() {
  ...
  if (screen === "playoff") return <Shell><div className="h-[100dvh] bg-slate-950 p-2 flex flex-col gap-2 overflow-hidden"><Card className="shrink-0 border border-purple-400/50 bg-gradient-to-br from-slate-900 via-slate-950 to-purple-950/70 p-2.5 shadow-2xl"><div className="flex items-start justify-between gap-3"><div className="min-w-0 flex-1"><p className="text-[8px] uppercase tracking-[0.2em] font-black text-purple-300 whitespace-nowrap">Double or Nothing</p><h1 className="mt-0.5 text-[23px] font-black leading-none text-white whitespace-nowrap">Full-Court Gamble</h1></div><button type="button" onClick={() => setScreen("lockerRoom")} className="shrink-0 rounded-xl border border-sky-300/50 bg-sky-500 px-2.5 py-1.5 text-[9px] font-black text-white shadow-lg active:scale-[0.98] hover:bg-sky-600">Locker Room</button></div><div className="mt-2 rounded-2xl border border-purple-300/25 bg-slate-950/65 p-2 text-center shadow-inner"><p className="text-[8px] uppercase tracking-[0.22em] font-black text-slate-500">Current Bank</p><p className="mt-0.5 text-3xl font-black leading-none text-white">{formatNumber(currentPoints)} <span className="text-sm text-slate-400">pts</span></p></div><p className="mt-1.5 text-center text-[11px] font-black leading-tight text-slate-200">One shot. Double your bank or lose it all.</p><p className="text-center text-[9px] font-bold leading-tight text-slate-500">All bonuses are off.</p></Card><Card className="relative flex-1 min-h-0 overflow-hidden border border-slate-700 bg-gradient-to-b from-slate-950 via-slate-900 to-[#c98542]"><div className="absolute left-3 top-3 z-20 rounded-2xl border border-purple-200/40 bg-slate-950/72 px-4 py-3 shadow-lg backdrop-blur-sm"><p className="text-[10px] uppercase tracking-[0.18em] font-black text-purple-200 leading-none">Make Chance</p><p className="mt-1 text-3xl font-black leading-none text-white">{doubleOrNothingChance}%</p></div>
