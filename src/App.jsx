...
function SingleTrackUpgradeCard({ title, theme = "lab", label, current, add, cost, level, max, onBuy, currentPoints, note = null, locked = false }) {
  ...
  const [notePlacement, setNotePlacement] = useState("above");
...
function ShotUpgradeCard({ title, theme = "lab", accuracyCurrent, accuracyAdd, accuracyCost, accuracyLevel, accuracyMax, onBuyAccuracy, valueCurrent, valueAdd, valueCost, valueLevel, valueMax, onBuyValue, currentPoints, locked = false, unlockCost = 0, onUnlock = null }) {
  ...
  const Row = ({ label, current, add, cost, level, max, onBuy }) => {
    const maxed = level >= max;
    const disabled = currentPoints < cost;
    return <div className="rounded-xl bg-slate-950/70 border border-slate-800 px-2.5 py-1.5"><div className="flex items-center justify-between gap-2"><div><p className="text-[10px] uppercase tracking-wide text-slate-500 leading-none">{label}</p><p className="text-sm font-black text-slate-100 mt-1 leading-tight">{current}</p></div><UpgradeButton onClick={onBuy} disabled={disabled || maxed} className={`rounded-lg px-3 py-1.5 text-[11px] font-black ${maxed ? "bg-emerald-700/40 text-emerald-200" : disabled ? "bg-slate-800 text-slate-500" : t.button}`}>{maxed ? "Maxed" : `Upgrade ${formatNumber(cost)} pts`}</UpgradeButton></div><div className="mt-2 flex items-center gap-2"><div className="h-2 flex-1 rounded-full bg-slate-800 overflow-hidden border border-slate-700"><div className={`h-full rounded-full ${t.bar}`} style={{ width: `${Math.min(100, (level / max) * 100)}%` }} /></div><p className="text-[10px] font-black text-slate-300 w-10 text-right">{level}/{max}</p></div><p className={`text-xs font-black mt-1 ${maxed ? "text-emerald-300" : t.add}`}>{maxed ? "Maxed" : add}</p></div>;
  };

  if (locked) {
    return <div className={`col-span-2 rounded-xl border shadow-sm p-2.5 ${t.card} relative overflow-hidden min-h-[196px]`}><div className="absolute inset-0 bg-slate-950/45 z-0" /><p className={`relative z-10 text-base font-black leading-tight ${t.title}`}>{title}</p><div className="relative z-10 mt-1.5 grid grid-cols-1 gap-1.5 opacity-50"><div className="rounded-xl bg-slate-950/45 border border-slate-800 px-3 py-2"><p className="text-[10px] uppercase tracking-wide text-slate-500 leading-none">Make Chance</p><p className="text-sm font-black text-slate-300 mt-1 leading-tight">{accuracyCurrent}</p><div className="mt-2 h-2 rounded-full bg-slate-800 border border-slate-700" /><p className="text-xs font-black mt-1 text-slate-400">{accuracyAdd}</p></div><div className="rounded-xl bg-slate-950/45 border border-slate-800 px-3 py-2"><p className="text-[10px] uppercase tracking-wide text-slate-500 leading-none">Shot Value</p><p className="text-sm font-black text-slate-300 mt-1 leading-tight">{valueCurrent}</p><div className="mt-2 h-2 rounded-full bg-slate-800 border border-slate-700" /><p className="text-xs font-black mt-1 text-slate-400">{valueAdd}</p></div></div><div className="absolute inset-x-0 top-14 bottom-2 z-20 flex items-center justify-center pointer-events-none"><button type="button" onClick={onUnlock} disabled={currentPoints < unlockCost} className={`pointer-events-auto w-28 h-28 rounded-3xl border-2 text-sm font-black shadow-xl transition active:scale-[0.98] ${currentPoints < unlockCost ? "bg-slate-800 border-slate-700 text-slate-500" : `${t.button} border-white/30`}`}><div className="flex h-full w-full flex-col items-center justify-center"><span className="text-2xl">{title === "Coach’s Challenge" ? "📋" : "🔒"}</span><span>Unlock</span><span className="text-sm mt-1">{formatNumber(unlockCost)} pts</span></div></button></div></div>;
  }

  return <div className={`col-span-2 rounded-xl border shadow-sm p-2.5 ${t.card}`}><p className={`text-base font-black leading-tight ${t.title}`}>{title}</p><div className="mt-1.5 grid grid-cols-1 gap-1.5"><Row label="Make Chance" current={accuracyCurrent} add={accuracyAdd} cost={accuracyCost} level={accuracyLevel} max={accuracyMax} onBuy={onBuyAccuracy} /><Row label="Shot Value" current={valueCurrent} add={valueAdd} cost={valueCost} level={valueLevel} max={valueMax} onBuy={onBuyValue} /></div></div>;
}

function TrophyCard({ title, text, buttonText, disabled, maxed, onClick, className, titleClassName, buttonClassName }) {
  return <div onClick={maxed ? onClick : undefined} className={`col-span-2 rounded-2xl border shadow-sm p-4 min-h-[190px] flex flex-col justify-between active:scale-[0.99] ${className}`}><div><p className={`text-xl font-black ${titleClassName}`}>{title}</p>{text ? <p className="mt-2 text-[12px] leading-snug text-slate-300">{text}</p> : null}</div><button type="button" disabled={disabled && !maxed} onClick={onClick} className={`mt-4 w-full rounded-2xl px-4 py-4 text-sm font-black ${maxed ? "bg-yellow-300 text-slate-950 shadow-[0_0_18px_rgba(250,204,21,0.45)]" : disabled ? "bg-slate-800 text-slate-500" : buttonClassName}`}>{maxed ? "View Crown Stats" : buttonText}</button></div>;
}
...
function CrownRevealTile({ name, value, detail, tone = "violet" }) {
  ...
  const accentMap = {
    green: "from-green-300 via-emerald-300 to-lime-200",
    red: "from-red-300 via-orange-300 to-yellow-200",
    yellow: "from-yellow-200 via-amber-300 to-orange-300",
    emerald: "from-emerald-300 via-teal-300 to-sky-300",
    white: "from-white via-yellow-100 to-sky-100",
    orange: "from-orange-300 via-amber-300 to-red-300",
    violet: "from-violet-300 via-fuchsia-300 to-sky-300",
  };
  const accentClass = accentMap[tone] || accentMap.violet;

  return (
    <button type="button" onClick={() => setFlipped((current) => !current)} className="h-[142px] rounded-2xl text-left active:scale-[0.98]" style={{ perspective: 900 }}>
      <motion.div animate={{ rotateY: flipped ? 180 : 0 }} transition={{ duration: 0.42, ease: "easeInOut" }} className="relative h-full w-full" style={{ transformStyle: "preserve-3d" }}>
        <div className={`absolute inset-0 rounded-2xl border ${toneClass} bg-slate-950 px-3 py-3 overflow-hidden shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]`} style={{ backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", transform: "rotateY(0deg)", zIndex: flipped ? 0 : 2 }}>
          <div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${accentClass}`} />
          <div className={`absolute -right-6 -top-7 h-20 w-20 rounded-full bg-gradient-to-br ${accentClass} opacity-20 blur-xl`} />
          <div className={`absolute -left-7 bottom-[-32px] h-20 w-20 rounded-full bg-gradient-to-br ${accentClass} opacity-10 blur-2xl`} />
          <p className="relative z-10 text-[7px] uppercase tracking-[0.14em] font-black text-slate-500">Bonus Credit</p>
          <p className="relative z-10 mt-1.5 text-[10px] font-black uppercase leading-tight tracking-[-0.02em] text-white">{name}</p>
          <p className={`relative z-10 mt-2.5 inline-block bg-gradient-to-r ${accentClass} bg-clip-text text-[19px] font-black leading-tight text-transparent break-words max-w-full drop-shadow-[0_0_10px_rgba(255,255,255,0.08)]`}>{value}</p>
          <p className="absolute bottom-2 right-2.5 text-[7px] font-black uppercase tracking-widest text-slate-600">Tap</p>
        </div>
        <div className={`absolute inset-0 rounded-2xl border ${toneClass} bg-slate-950 px-3 py-3 overflow-hidden shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]`} style={{ backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", transform: "rotateY(180deg)", zIndex: flipped ? 2 : 0 }}>
          <div className={`absolute inset-x-0 top-0 h-1 bg-gradient-to-r ${accentClass}`} />
          <div className={`absolute -right-6 -top-7 h-20 w-20 rounded-full bg-gradient-to-br ${accentClass} opacity-16 blur-xl`} />
          <p className="relative z-10 text-[12px] font-black leading-tight text-white">{name}</p>
          <p className="relative z-10 mt-2 text-[11px] font-bold leading-snug text-slate-300">{detail}</p>
        </div>
      </motion.div>
    </button>
  );
...
export default function BasketballGame() {
  ...
  }, [hasWon, currentPoints, bestStreak, careerStats, tripPointsEarned, courtRound]);
  ...
  function useCoachChallenge() {
    ...
    window.setTimeout(() => {
      ...
      setOpenLook(savedChallenge.openLookBefore || null);
...
Erase the mistake, retake the shot, and keep the run alive.`, cost: COACH_CHALLENGE_COST, level: upgrades.coachChallenge, max: 1, action: () => buyUpgrade("coachChallenge", COACH_CHALLENGE_COST, 1) },
  ...
  if (screen === "crownCelebration") return (
    <Shell>
      <motion.div animate={justClaimedCrown ? { x: [0, -8, 8, -6, 6, -3, 3, 0], y: [0, 4, -4, 2, -2, 0] } : { x: 0, y: 0 }} transition={{ duration: 0.62, ease: "easeOut" }} className="h-[100dvh] relative overflow-hidden bg-gradient-to-b from-slate-950 via-yellow-950/20 to-slate-950 p-2 flex items-start justify-center">
        ...
        <Card className="relative w-full h-[98dvh] mt-0 border border-yellow-300/45 bg-slate-950 p-2 text-center shadow-[0_0_42px_rgba(250,204,21,0.18)] overflow-hidden flex flex-col">
          ...
          <div className="shrink-0 relative z-10">
            <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              ...
              <p className="mt-1 rounded-2xl border border-yellow-300/20 bg-yellow-300/8 px-3 py-1 text-[9px] font-black leading-snug text-yellow-100">You finished the grind and earned the crown.</p>
            </motion.div>
          </div>
          ...
          <div className="relative z-10 mt-1.5 rounded-[28px] border border-yellow-300/20 bg-[radial-gradient(circle_at_top,rgba(250,204,21,0.13),rgba(15,23,42,0.94)_42%,rgba(2,6,23,0.98))] p-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.08),0_0_30px_rgba(250,204,21,0.08)] overflow-hidden flex-1 min-h-0">
            ...
            <p className="mb-1.5 text-[9px] font-bold leading-snug text-slate-400 text-center">The grind is over. Tap each card for the bonus scenes from your run.</p>
  ...
  if (screen === "playoff") return (
    <Shell>
      <div className="h-[100dvh] bg-slate-950 p-2 flex flex-col gap-2 overflow-hidden">
        <Card className="shrink-0 border border-slate-700 bg-slate-950 p-3 shadow-2xl">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[8px] uppercase tracking-[0.22em] font-black text-slate-500">Current Points</p>
              <p className="mt-0.5 text-3xl font-black leading-none text-white">{formatNumber(currentPoints)} <span className="text-sm text-slate-400">pts</span></p>
              {activeWager > 0 && <p className="mt-1 text-[11px] font-black uppercase tracking-wide text-purple-300">Wager: {formatNumber(activeWager)} pts</p>}
            </div>
            <button type="button" onClick={() => setScreen("lockerRoom")} className="rounded-xl border border-sky-300/50 bg-sky-500 px-3 py-2 text-[10px] font-black text-white shadow-lg active:scale-[0.98] hover:bg-sky-600">Locker Room</button>
          </div>
        </Card>

        <Card className="relative flex-1 min-h-0 overflow-hidden border border-slate-700 bg-gradient-to-b from-slate-950 via-[#1a1111] to-[#c98542]">
          <div className="absolute inset-0 opacity-60" style={{ backgroundImage: "linear-gradient(90deg, rgba(255,255,255,0.07) 1px, transparent 1px), linear-gradient(0deg, rgba(255,255,255,0.05) 1px, transparent 1px)", backgroundSize: "10% 8%" }} />

          <div className="absolute left-3 top-3 z-20 rounded-2xl border border-purple-200/40 bg-slate-950/72 px-4 py-3 shadow-lg backdrop-blur-sm">
            <p className="text-[10px] uppercase tracking-[0.18em] font-black text-purple-200 leading-none">Make Chance</p>
            <p className="mt-1 text-3xl font-black leading-none text-white">{doubleOrNothingChance}%</p>
          </div>

          <div className="absolute left-1/2 top-[15%] z-10 -translate-x-1/2 pointer-events-none">
            <div className="relative h-[36px] w-[62px] rounded-[4px] border-[3px] border-white bg-slate-100 shadow-[0_4px_10px_rgba(0,0,0,0.35)]">
              <div className="absolute left-1/2 top-[8px] h-[12px] w-[18px] -translate-x-1/2 border-[2px] border-slate-500" />
              <div className="absolute left-1/2 top-[24px] h-[6px] w-[30px] -translate-x-1/2 rounded-full border-[3px] border-orange-500 bg-transparent shadow-[0_0_8px_rgba(249,115,22,0.55)]" />
              <div className="absolute left-1/2 top-[29px] -translate-x-1/2">
                <svg width="30" height="20" viewBox="0 0 30 20">
                  <path d="M3 1 L7 18 M9 1 L12 18 M15 1 L15 18 M21 1 L18 18 M27 1 L23 18" stroke="rgba(255,255,255,0.95)" strokeWidth="1.2" />
                  <path d="M3 1 H27" stroke="rgba(255,255,255,0.95)" strokeWidth="1.2" />
                  <path d="M5 7 H25 M7 12 H23 M10 16 H20" stroke="rgba(255,255,255,0.78)" strokeWidth="1" />
                </svg>
              </div>
            </div>
          </div>

          {!playoffShot && !playoffResult && <div className="absolute left-1/2 bottom-[22%] z-10 -translate-x-1/2 pointer-events-none"><div className="relative h-14 w-14 rounded-full border-4 border-orange-900 bg-orange-500 shadow-2xl opacity-85 overflow-hidden"><div className="absolute inset-y-0 left-1/2 w-1 -translate-x-1/2 bg-orange-900/75" /><div className="absolute inset-x-0 top-1/2 h-1 -translate-y-1/2 bg-orange-900/75" /><div className="absolute inset-2 rounded-full border border-orange-900/60" /></div></div>}

          {!playoffShot && !playoffResult && activeWager <= 0 && (
            <div className="absolute inset-x-4 bottom-5 z-20">
              <button type="button" onClick={() => { setWagerInput("0"); setShowWagerModal(true); }} className="w-full rounded-3xl border-2 border-purple-200 bg-purple-500 px-8 py-4 text-lg font-black uppercase tracking-wide text-white shadow-[0_0_24px_rgba(168,85,247,0.45)] active:scale-[0.98]">Place Wager</button>
            </div>
          )}

          {!playoffShot && !playoffResult && activeWager > 0 && (
            <div className="absolute inset-x-4 bottom-5 z-20 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => { setWagerInput(String(activeWager)); setShowWagerModal(true); }} className="rounded-2xl border-2 border-purple-200/70 bg-slate-950 px-3 py-4 text-sm font-black uppercase tracking-wide text-purple-100 shadow-[0_0_20px_rgba(168,85,247,0.25)] active:scale-[0.98]">Change Wager</button>
              <button type="button" onClick={takePlayoffShot} className="rounded-2xl border-2 border-orange-200 bg-orange-500 px-3 py-4 text-sm font-black uppercase tracking-wide text-white shadow-[0_0_20px_rgba(249,115,22,0.45)] active:scale-[0.98]">Shoot the Ball</button>
            </div>
          )}

          <AnimatePresence>{playoffShot && <motion.div key={playoffShot.key} initial={{ left: "50%", top: "78%", rotate: 0 }} animate={playoffShot.made ? { left: ["50%", "50%", "50%", "50%", "50%", `${PLAYOFF_BACK_RIM_TARGET.x}%`, "50%"], top: ["78%", "38%", "-18%", "-18%", "9%", `${PLAYOFF_BACK_RIM_TARGET.y}%`, "32%"], rotate: [0, 120, 240, 240, 320, 420, 500] } : playoffShot.missSide === "left" ? { left: ["50%", "50%", "50%", "50%", "50%", "45%", "37%"], top: ["78%", "38%", "-18%", "-18%", "9%", "25%", "32%"], rotate: [0, 120, 240, 240, 320, 420, 520] } : { left: ["50%", "50%", "50%", "50%", "50%", "55%", "63%"], top: ["78%", "38%", "-18%", "-18%", "9%", "25%", "32%"], rotate: [0, 120, 240, 240, 320, 420, 520] }} transition={{ duration: 7.8, ease: "easeInOut", times: [0, 0.28, 0.46, 0.58, 0.68, 0.86, 1] }} style={{ x: "-50%", y: "-50%" }} className="absolute z-30 pointer-events-none h-12 w-12"><motion.div animate={playoffShot.made ? { scale: [1.35, 0.95, 0.7, 0.7, 0.34, 0.2, 0.06], opacity: [1, 1, 0, 0, 1, 1, 0] } : { scale: [1.35, 0.95, 0.7, 0.7, 0.34, 0.22, 0.16], opacity: [1, 1, 0, 0, 1, 1, 0] }} transition={{ duration: 7.8, ease: "easeInOut", times: [0, 0.28, 0.46, 0.58, 0.68, 0.86, 1] }} className="relative h-12 w-12 rounded-full border-4 border-orange-900 bg-orange-500 shadow-2xl overflow-hidden"><div className="absolute inset-y-0 left-1/2 w-1 -translate-x-1/2 bg-orange-900/75" /><div className="absolute inset-x-0 top-1/2 h-1 -translate-y-1/2 bg-orange-900/75" /><div className="absolute inset-2 rounded-full border border-orange-900/60" /></motion.div></motion.div>}</AnimatePresence>

          <AnimatePresence>{playoffResult && <motion.div initial={{ opacity: 0, scale: 0.55, y: 18 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ type: "spring", stiffness: 180, damping: 14 }} className={`absolute inset-0 z-40 flex flex-col items-center justify-center text-center pointer-events-none ${playoffResult === "made" ? "text-green-300" : "text-red-300"}`}>{playoffResult === "made" ? <><div className="mb-3 rounded-full border-2 border-green-200/60 bg-green-400/20 px-5 py-2 text-2xl font-black text-green-100 shadow-[0_0_28px_rgba(134,239,172,0.55)]">{playoffShot?.phrase || "CASHED IT!"}</div><div className="text-5xl font-black drop-shadow-[0_0_18px_rgba(134,239,172,0.8)]">WAGER WON</div><div className="mt-2 text-2xl font-black text-green-100">+{formatNumber(playoffShot?.wagerAmount || 0)} pts</div></> : <><div className="mb-3 rounded-full border-2 border-red-200/60 bg-red-400/20 px-5 py-2 text-2xl font-black text-red-100 shadow-[0_0_28px_rgba(248,113,113,0.45)]">{playoffShot?.phrase || "BUST"}</div><div className="text-6xl font-black">WAGER LOST</div><div className="mt-2 text-2xl font-black text-red-100">-{formatNumber(playoffShot?.wagerAmount || 0)} pts</div></>}</motion.div>}</AnimatePresence>

          <AnimatePresence>{showWagerModal && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-[90] flex items-center justify-center bg-slate-950/80 px-4 backdrop-blur-sm">
            <motion.div initial={{ y: 22, scale: 0.94 }} animate={{ y: 0, scale: 1 }} exit={{ y: 18, scale: 0.96 }} className="w-full max-w-[340px] rounded-[30px] border-2 border-purple-200/70 bg-slate-950 p-5 text-center shadow-[0_0_40px_rgba(168,85,247,0.35)]">
              <p className="text-[10px] uppercase tracking-[0.28em] font-black text-purple-300">DeepCourt Gambit</p>
              <h2 className="mt-2 text-3xl font-black uppercase leading-none text-white">Place Wager</h2>
              <p className="mt-3 rounded-2xl border border-purple-300/25 bg-purple-500/10 px-3 py-3 text-[13px] font-bold leading-snug text-slate-200">Pick how many points to risk. Make the shot and you win that amount. Miss and those points are gone.</p>
              <div className="mt-4 rounded-2xl border border-slate-700 bg-slate-900 p-3">
                <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-wide text-slate-500"><span>Amount</span><span>Max {formatNumber(currentPoints)}</span></div>
                <input type="number" min="0" max={currentPoints} value={wagerInput} onChange={(event) => setWagerInput(String(Math.max(0, Math.min(currentPoints, Math.floor(Number(event.target.value) || 0)))))} className="mt-2 w-full rounded-2xl border border-purple-300/40 bg-slate-950 px-4 py-3 text-center text-3xl font-black text-white outline-none focus:border-purple-200" />
                <button type="button" onClick={() => setWagerInput(String(currentPoints))} className="mt-2 rounded-xl border border-purple-300/30 bg-purple-500/15 px-3 py-2 text-[10px] font-black uppercase tracking-wide text-purple-200 active:scale-[0.98]">Max Wager</button>
              </div>
              <div className="mt-4 grid grid-cols-2 gap-2">
                <button type="button" onClick={() => setShowWagerModal(false)} className="rounded-2xl border border-slate-600 bg-slate-900 px-4 py-3 text-sm font-black uppercase text-slate-200 active:scale-[0.98]">Cancel</button>
                <button type="button" disabled={getPendingWagerAmount() <= 0} onClick={() => { if (getPendingWagerAmount() <= 0) return; setActiveWager(getPendingWagerAmount()); setShowWagerModal(false); }} className={`rounded-2xl border px-4 py-3 text-sm font-black uppercase shadow-[0_0_18px_rgba(168,85,247,0.45)] active:scale-[0.98] ${getPendingWagerAmount() <= 0 ? "border-slate-700 bg-slate-800 text-slate-500" : "border-purple-200 bg-purple-500 text-white"}`}>Wager</button>
              </div>
            </motion.div>
  ...
  if (screen === "lockerRoom") return (
    <Shell>
      {transitionOverlay}
      <Card onClick={() => setConfirmDoubleOrNothingUpgrade(false)} className="h-[100dvh] p-2.5 bg-gradient-to-b from-slate-900 to-slate-950 flex flex-col overflow-hidden touch-pan-y">
        <div className="shrink-0 rounded-3xl overflow-hidden border border-slate-700 bg-gradient-to-b from-slate-800 to-slate-950 px-3 pt-1.5 pb-3 shadow-xl">
          <div className="flex items-start justify-between gap-3">
            <div>
              <button type="button" onClick={() => setScreen("title")} className="mb-0.5 rounded-md border border-slate-700 bg-slate-950/70 px-2 py-0 text-[8px] font-black uppercase tracking-wide text-slate-400 shadow-sm active:scale-[0.98]">Main Menu</button>
              <h1 className="text-3xl font-black leading-none mt-0.5">{formatNumber(currentPoints)} <span className="text-sm font-black text-slate-400 align-middle">pts</span></h1>
            </div>
            <button type="button" onClick={startCourt} className="rounded-2xl bg-orange-500 hover:bg-orange-600 text-white font-black px-4 py-3 shadow-xl active:scale-[0.98]">Hit the Court</button>
          </div>

          <div className="grid grid-cols-6 gap-1 mt-1">
            <StatBox label="Balls" value={shotsPerTrip} color="text-sky-300" maxed={upgrades.extraShots >= EXTRA_BALL_COSTS.length} maxTheme="sky" labelSize="text-[7px]" valueSize="text-[13px]" heightClass="h-[52px]" onClick={() => handleLockerCheatTap("balls")} />
            <StatBox label="Hot Hand" value={upgrades.hotHand > 0 ? `x${HOT_HAND_MULTS[upgrades.hotHand - 1]}` : "-"} color={upgrades.hotHand > 0 ? "text-red-300" : "text-slate-500"} maxed={upgrades.hotHand >= HOT_HAND_COSTS.length} maxTheme="red" labelSize="text-[7px]" valueSize="text-[13px]" heightClass="h-[52px]" onClick={() => handleLockerCheatTap("hot")} />
            <StatBox label="Golden" value={upgrades.goldenBall > 0 ? (upgrades.superGolden > 0 ? <span className="flex flex-col items-center leading-none"><span>{goldenChance}%</span><span className="mt-0.5 text-[7px] uppercase tracking-wide text-white">+SG</span></span> : `${goldenChance}%`) : "-"} color={upgrades.goldenBall > 0 ? "text-yellow-300" : "text-slate-500"} maxed={upgrades.goldenBall >= GOLDEN_BALL_COSTS.length} maxTheme={upgrades.superGolden > 0 ? "superGold" : "yellow"} labelSize="text-[7px]" valueSize="text-[11px]" heightClass="h-[52px]" onClick={() => handleLockerCheatTap("golden")} />
            <StatBox label="Rim Rescue" value={upgrades.doubleRim > 0 ? `${doubleRimChance}%` : "-"} color={upgrades.doubleRim > 0 ? "text-emerald-300" : "text-slate-500"} maxed={upgrades.doubleRim >= DOUBLE_RIM_COSTS.length} maxTheme="emerald" labelSize="text-[7px]" valueSize="text-[11px]" heightClass="h-[52px]" onClick={() => handleLockerCheatTap("rim")} />
            <StatBox label="Open Look" value={upgrades.courtVision > 0 ? (openLookCooldown ? "-" : `${courtVisionChance}%`) : "-"} color={upgrades.courtVision > 0 ? (openLookCooldown ? "text-slate-500" : "text-lime-300") : "text-slate-500"} maxed={upgrades.courtVision >= COURT_VISION_COSTS.length && !openLookCooldown} maxTheme="emerald" labelSize="text-[7px]" valueSize="text-[11px]" heightClass="h-[52px]" onClick={() => handleLockerCheatTap("look")} />
            <div onClick={() => handleLockerCheatTap("challenge")} className={`h-[52px] rounded-xl px-1.5 py-1 text-center relative overflow-hidden flex flex-col items-center justify-center border active:scale-[0.98] ${upgrades.coachChallenge > 0 ? "bg-cyan-950/70 border-cyan-300/70 text-cyan-100 shadow-[0_0_16px_rgba(34,211,238,0.45)]" : "bg-slate-900/80 border-slate-700 text-slate-500"}`}>
        ...
        <div className="mt-2 grid grid-cols-3 gap-1.5">
          <button type="button" onClick={() => setLockerScreen("shots")} className={`rounded-xl border px-2 py-2 text-[10px] font-black active:scale-[0.98] ${lockerScreen === "shots" ? "bg-orange-500 border-orange-300 text-white shadow-[0_0_14px_rgba(249,115,22,0.45)]" : "bg-slate-950 border-slate-700 text-slate-400"}`}>
            <div className="flex flex-col items-center justify-center gap-1"><span className="text-base leading-none">🏀</span><span className="leading-none">Shot Lab</span></div>
          </button>
          <button type="button" onClick={() => setLockerScreen("bonus")} className={`rounded-xl border px-2 py-2 text-[10px] font-black active:scale-[0.98] ${lockerScreen === "bonus" ? "bg-red-500 border-red-300 text-white shadow-[0_0_14px_rgba(239,68,68,0.45)]" : "bg-slate-950 border-slate-700 text-slate-400"}`}>
            <div className="flex flex-col items-center justify-center gap-1"><span className="text-base leading-none">⚡</span><span className="leading-none">Power-Ups</span></div>
          </button>
          <button type="button" onClick={() => setLockerScreen("mystery")} className={`rounded-xl border px-2 py-2 text-[10px] font-black active:scale-[0.98] ${lockerScreen === "mystery" ? "bg-violet-500 border-violet-300 text-white shadow-[0_0_14px_rgba(139,92,246,0.45)]" : "bg-slate-950 border-slate-700 text-slate-400"}`}>
            <div className="flex flex-col items-center justify-center gap-1"><span className="text-base leading-none">🎲</span><span className="leading-none">High Stakes</span></div>
          </button>
        </div>

        <div className="mt-2 flex-1 min-h-0 overflow-y-scroll overscroll-contain touch-pan-y pr-1 [-webkit-overflow-scrolling:touch]">
          <div className="grid grid-cols-2 gap-2 pb-0">
            {shopItems.filter(Boolean).filter((item) => {
              ...
              if (item.type === "section") {
                ...
                return <div key={`section-${index}`} className={`col-span-2 mt-2 first:mt-0 rounded-xl border px-3 py-2 ${t.header}`}><p className="text-[9px] uppercase tracking-[0.2em] font-black">{item.title}</p></div>;
              }
              if (item.type === "singleTrack") return <SingleTrackUpgradeCard key={item.key} {...item} onBuy={item.action} currentPoints={currentPoints} />;
              if (item.type === "shotUpgrade") return <ShotUpgradeCard key={item.key} title={item.title} theme="lab" accuracyCurrent={item.accuracyCurrent} accuracyAdd={item.accuracyAdd} accuracyCost={item.accuracyCost} accuracyLevel={item.accuracyLevel} accuracyMax={item.accuracyMax} onBuyAccuracy={item.accuracyAction} valueCurrent={item.valueCurrent} valueAdd={item.valueAdd} valueCost={item.valueCost} valueLevel={item.valueLevel} valueMax={item.valueMax} onBuyValue={item.valueAction} currentPoints={currentPoints} locked={!!item.locked} unlockCost={item.unlockCost ?? 0} onUnlock={item.unlockAction} />;
              if (item.type === "playoff") return <div key="playoff" onClick={(event) => event.stopPropagation()} className="col-span-2 rounded-xl border border-white/20 bg-[linear-gradient(135deg,rgba(15,23,42,0.96),rgba(88,28,135,0.28),rgba(30,64,175,0.22),rgba(5,150,105,0.18),rgba(202,138,4,0.18),rgba(190,24,93,0.2))] p-3.5 shadow-[0_0_24px_rgba(255,255,255,0.06)]"><div className="flex items-start justify-between gap-3"><div><p className="text-[15px] font-black text-white">DeepCourt Gambit</p><p className="mt-1 text-[11px] leading-snug text-slate-300">Enter the challenge, place your wager, and take the deep shot.</p></div><div className="rounded-xl border border-white/20 bg-white/10 px-2.5 py-1.5 text-center"><p className="text-lg font-black leading-none text-white">{currentPoints >= PLAYOFF_TICKET_COST ? `${doubleOrNothingChance}%` : "?"}</p><p className="text-[7px] uppercase tracking-wide font-black text-slate-300">Make</p></div></div><button type="button" disabled={currentPoints < PLAYOFF_TICKET_COST} onClick={openPlayoffDoor} className={`mt-3 w-full rounded-xl px-3 py-2 text-xs font-black ${currentPoints < PLAYOFF_TICKET_COST ? "bg-slate-800 text-slate-500" : "bg-white/12 hover:bg-white/18 text-white border border-white/20"}`}>{currentPoints >= PLAYOFF_TICKET_COST ? "Enter Challenge" : `Reach ${formatNumber(PLAYOFF_TICKET_COST)} pts`}</button>{currentPoints >= PLAYOFF_TICKET_COST && <><div className="my-3 flex items-center gap-2"><div className="h-px flex-1 bg-white/12" /><span className="text-[8px] uppercase tracking-[0.18em] font-black text-slate-400">Upgrade</span><div className="h-px flex-1 bg-white/12" /></div><button type="button" disabled={upgrades.doubleOrNothing > 0 || currentPoints < DOUBLE_OR_NOTHING_UPGRADE_COST} onClick={buyDoubleOrNothingUpgrade} className={`mt-2 w-full rounded-xl px-3 py-2 text-xs font-black ${upgrades.doubleOrNothing > 0 ? "bg-emerald-700/40 text-emerald-200" : currentPoints < DOUBLE_OR_NOTHING_UPGRADE_COST ? "bg-slate-800 text-slate-500" : confirmDoubleOrNothingUpgrade ? "bg-red-500 hover:bg-red-600 text-white" : "bg-violet-500 hover:bg-violet-600 text-white"}`}>{upgrades.doubleOrNothing > 0 ? "50% Unlocked" : confirmDoubleOrNothingUpgrade ? "Are you sure? Tap again" : `Upgrade to 50% • ${formatNumber(DOUBLE_OR_NOTHING_UPGRADE_COST)} pts`}</button></>}</div>;
              if (item.type === "trophy") return <TrophyCard key="trophy" title="The Crown" text={hasWon ? "Crown claimed. Tap here anytime to revisit your winner stats." : "The finish line. Reach the mark, claim the crown, win the grind."} buttonText={currentPoints >= TROPHY_COST ? "Claim the Crown" : `Reach ${formatNumber(TROPHY_COST)} pts`} disabled={currentPoints < TROPHY_COST} maxed={hasWon} onClick={hasWon ? openCrownStats : buyTrophy} className="bg-gradient-to-br from-yellow-300/22 via-amber-400/22 to-orange-500/20 border-yellow-300/55 shadow-[0_0_28px_rgba(250,204,21,0.16)]" titleClassName="text-yellow-200" buttonClassName="bg-yellow-400 hover:bg-yellow-500 text-slate-950" />;
              return null;
            })}
          </div>
        </div>
