...
function StatBox({ label, value, color = "text-white", maxed = false, valueSize = "text-sm", onClick = undefined, maxTheme = "emerald", labelSize = "text-[8px]", heightClass = "h-[54px]" }) {
...
function TrophyCard({ title, text, buttonText, disabled, maxed, onClick, className, titleClassName, buttonClassName }) {
  return <div onClick={maxed ? onClick : undefined} className={`col-span-2 rounded-2xl border shadow-sm p-4 min-h-[190px] flex flex-col justify-between active:scale-[0.99] ${className}`}>
    <div>
      <p className={`text-xl font-black ${titleClassName}`}>{title}</p>
      {text ? <p className="mt-2 text-[12px] leading-snug text-slate-300">{text}</p> : null}
    </div>
    <button type="button" disabled={disabled && !maxed} onClick={onClick} className={`mt-4 w-full rounded-2xl px-4 py-4 text-sm font-black ${maxed ? "bg-yellow-300 text-slate-950 shadow-[0_0_18px_rgba(250,204,21,0.45)]" : disabled ? "bg-slate-800 text-slate-500" : buttonClassName}`}>{maxed ? "View Crown Stats" : buttonText}</button>
  </div>;
}
...
export default function BasketballGame() {
  ...
  if (screen === "lockerRoom") return <Shell>{transitionOverlay}<Card onClick={() => setConfirmDoubleOrNothingUpgrade(false)} className="h-[100dvh] p-2.5 bg-gradient-to-b from-slate-900 to-slate-950 flex flex-col overflow-hidden touch-pan-y"><div className="shrink-0 rounded-3xl overflow-hidden border border-slate-700 bg-gradient-to-b from-slate-800 to-slate-950 px-3 pt-1.5 pb-3 shadow-xl"><div className="flex items-start justify-between gap-3"><div><button type="button" onClick={() => setScreen("title")} className="mb-0.5 rounded-md border border-slate-700 bg-slate-950/70 px-2 py-0 text-[8px] font-black uppercase tracking-wide text-slate-400 shadow-sm active:scale-[0.98]">Main Menu</button><h1 className="text-3xl font-black leading-none mt-0.5">{formatNumber(currentPoints)} <span className="text-sm font-black text-slate-400 align-middle">pts</span></h1></div><button type="button" onClick={startCourt} className="rounded-2xl bg-orange-500 hover:bg-orange-600 text-white font-black px-4 py-3 shadow-xl active:scale-[0.98]">Hit the Court</button></div><div className="grid grid-cols-6 gap-1 mt-1">
        ...
        <div className="mt-2 grid grid-cols-3 gap-1.5">
            <button type="button" onClick={() => setLockerScreen("shots")} className={`min-w-0 rounded-xl border px-1 py-1.5 text-[9px] font-black active:scale-[0.98] ${lockerScreen === "shots" ? "bg-orange-500 border-orange-300 text-white shadow-[0_0_14px_rgba(249,115,22,0.45)]" : "bg-slate-950 border-slate-700 text-slate-400"}`}>
              <div className="flex min-w-0 flex-col items-center justify-center gap-0.5">
                <span className="text-sm leading-none">🏀</span>
                <span className="leading-none whitespace-nowrap">Shot Lab</span>
              </div>
            </button>
            <button type="button" onClick={() => setLockerScreen("bonus")} className={`min-w-0 rounded-xl border px-1 py-1.5 text-[9px] font-black active:scale-[0.98] ${lockerScreen === "bonus" ? "bg-red-500 border-red-300 text-white shadow-[0_0_14px_rgba(239,68,68,0.45)]" : "bg-slate-950 border-slate-700 text-slate-400"}`}>
              <div className="flex min-w-0 flex-col items-center justify-center gap-0.5">
                <span className="text-sm leading-none">⚡</span>
                <span className="leading-none whitespace-nowrap">Power-Ups</span>
              </div>
            </button>
            <button type="button" onClick={() => setLockerScreen("mystery")} className={`min-w-0 rounded-xl border px-1 py-1.5 text-[9px] font-black active:scale-[0.98] ${lockerScreen === "mystery" ? "bg-violet-500 border-violet-300 text-white shadow-[0_0_14px_rgba(139,92,246,0.45)]" : "bg-slate-950 border-slate-700 text-slate-400"}`}>
              <div className="flex min-w-0 flex-col items-center justify-center gap-0.5">
                <span className="text-sm leading-none">🎲</span>
                <span className="leading-none whitespace-nowrap">High Stakes</span>
              </div>
            </button>
